...
#         outputs = model(data)
        classes, bag_prediction, _, _ = model(data)
    
#         print('Inside train_one_epoch, classes is {} shape {}'.format(classes, classes.shape))
#         print('Inside train_one_epoch, bag_prediction is {} shape {}'.format(bag_prediction, bag_prediction.shape))
    
        max_prediction, index = torch.max(classes, 0)
#         print('Inside train_one_epoch, max_prediction is {} shape {}'.format(max_prediction, max_prediction.shape))

        loss_bag = F.cross_entropy(bag_prediction, bag_label, weights, reduction='mean')
#         loss_bag = criterion(bag_prediction.view(1, -1), label.view(1, -1))
#         loss_max = criterion(max_prediction.view(1, -1), label.view(1, -1))
        
        loss_max = F.cross_entropy(max_prediction.view(1, -1), bag_label, weights, reduction='mean')
        LabeledCELoss = 0.5*loss_bag + 0.5*loss_max
...
#             ema_outputs, _ = ema_model(data)
            _, raw_outputs, _, _ = raw_model(data)
            _, ema_outputs, _, _ = ema_model(data)
